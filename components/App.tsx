...
import FocusModeView from './nexus/FocusModeView';
import FinanceView from './FinanceView';
import NotificationsView from './nexus/NotificationsView';
import LeadsHub from './system/LeadsHub';
import HeadquartersView from './nexus/HeadquartersView';
import WorkspaceHub from './nexus/WorkspaceHub';
import OperationsHub from './nexus/OperationsHub';
import SystemHub from './nexus/SystemHub';
import ReportsView from './nexus/ReportsView';
import ErrorBoundary from './ErrorBoundary';
import LoginView from './LoginView';
import ClientPortalView from './ClientPortalView';
import PersonalAreaView from './nexus/PersonalAreaView';
...
const SystemOSApp = () => {
  ...
  const getHeaderActions = () => {
      switch(activeTab) {
          ...
          case 'sales_hub':
              return (
                  <button onClick={() => setShowNewLeadModal(true)} className="bg-onyx-900 hover:bg-black text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg transition-all flex items-center gap-2">
                      <UserPlus size={16} /> <span className="hidden md:inline">ליד חדש</span>
                  </button>
              );
          case 'finance':
              return (
                  <button onClick={() => addToast('פעולות כספיות', 'info')} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg transition-all flex items-center gap-2">
                      <FileText size={16} /> <span className="hidden md:inline">פעולה כספית</span>
                  </button>
              );
          case 'marketing':
              return (
                  <button onClick={() => setShowNewLeadModal(true)} className="bg-nexus-gradient hover:opacity-90 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg transition-all flex items-center gap-2">
