...
export const TaskDetailChat: React.FC<TaskDetailChatProps> = ({ task, activeTab }) => {
    ...
    }, [task.messages, activeTab, isRecordingComment, isTranscribing]);
            ...
            mediaRecorderRef.current.onstop = async () => {
                ...
                try {
                    if (process.env.API_KEY) {
                            ...
                            const response = await ai.models.generateContent({
                                ...
                                contents: {
                                    parts: [
                                        ...
                                        { text: "Transcribe the following audio to text in Hebrew (or the spoken language). Return ONLY the transcribed text, nothing else." }
                                    ]
                                }
                            });
                        ...
                        setTimeout(() => {
                            setMessageText(prev => prev + " (תמלול: בדוק מפתח API)");
                            setIsTranscribing(false);
                        }, 1000);
                    }
                } catch (e) {
        ...
        <div className={`w-full md:w-[35%] md:shrink-0 flex flex-col bg-[#e5ddd5] relative border-l border-gray-200 overflow-hidden ${activeTab === 'chat' ? 'flex-1 h-full min-h-0' : 'hidden md:flex h-full'}`}>
            ...
            <div className="hidden md:flex p-4 border-b border-gray-200 bg-[#f0f2f5] z-10 justify-between items-center sticky top-0 shadow-sm">
                <h3 className="font-bold text-gray-800 flex items-center gap-2 text-sm">
                    ...
                    <span className="bg-green-500 text-white px-2 py-0.5 rounded-full text-xs font-bold shadow-sm">{task.messages.length}</span>
                </h3>
            </div>
