...
interface FinanceViewProps {
    ...
    initialTab?: 'commission' | 'mrr' | 'invoices' | 'catalog';
}

const FinanceView: React.FC<FinanceViewProps> = ({ invoices = [], onAddInvoice, onUpdateInvoice, leads = [], initialTab = 'mrr' }) => {
    const { user } = useAuth();
    const { addToast } = useToast();
    const [activeTab, setActiveTab] = useState<'commission' | 'mrr' | 'invoices' | 'catalog'>(initialTab);
    ...
    const activeSubscriptions: Lead[] = [];
    ...
    const stats = useMemo(() => {
        ...
        const pending = invoices
            ...
            .reduce((sum, i) => sum + (i.amount || 0), 0);
        return { net: paid, gross: paid, refunded: 0, pending };
    }, [invoices]);
    ...
    const handleRefund = (invoice: Invoice) => {
        ...
        const invoiceLabel = (invoice as unknown as Record<string, unknown>).number ?? invoice.id;
                            ...
                            <div className="ui-card p-8 bg-onyx-900 text-white border-none shadow-2xl relative overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="mt-4 flex items-center gap-2 text-emerald-400 text-xs font-bold">
                                        <ArrowUpRight size={14} /> 0% מהחודש שעבר
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {activeSubscriptions.map(client => (
                                            <tr key={client.id} className="hover:bg-slate-50/50 transition-colors group">
                                                ...
                                                <td className="px-6 py-4 font-mono font-bold text-indigo-600">₪0</td>
