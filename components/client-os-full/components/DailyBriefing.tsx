...
import { X, Play, Circle, Calendar, Briefcase, Zap, CircleCheckBig } from 'lucide-react';
...
const DailyBriefing: React.FC<DailyBriefingProps> = ({ isOpen, onClose }) => {
    ...
    const { clients, meetings } = useNexus();

    const loadBriefing = async () => {
        const riskyCount = clients.filter(c => c.healthStatus === HealthStatus.CRITICAL || c.healthStatus === HealthStatus.AT_RISK).length;
        const oppCount = clients.flatMap(c => c.opportunities).length;
        const today = new Date().toLocaleDateString('he-IL');
        const meetingCount = meetings.filter(m => m.date === today).length;
        ...
        try {
            ...
            setTimeout(() => {
                setIsLoading(false);
                setTimeout(() => setShowContent(true), 500); // Slight delay for animation
            }, 1000);
        } catch (e) {
            console.error(e);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl animate-fade-in text-white font-sans">
            <div className="w-full max-w-3xl relative">
                ...
                {isLoading ? (
                    ...
                    <div className={`space-y-12 transition-all duration-1000 transform ${showContent ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                            ...
                            {briefing?.focusPoints?.map((point, idx) => (
                                ...
                                >
                                    <div className="mt-1 p-3 rounded-xl bg-gradient-to-br from-gray-800 to-black border border-white/20 shadow-lg group-hover:shadow-nexus-accent/20 group-hover:border-nexus-accent transition-all">
                                        {idx === 0 ? <Zap className="text-signal-warning" size={20} /> : 
                                         idx === 1 ? <Calendar className="text-blue-400" size={20} /> : 
