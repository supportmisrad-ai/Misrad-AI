...
export const PortalManagementTab: React.FC<PortalManagementTabProps> = ({ client }) => {
  ...
  const handleAddTask = async () => {
      ...
      await addClientTask(client.id, {
          ...newTask,
          dueDate: 'ASAP'
      });
      setShowTaskForm(false);
      setNewTask({ title: '', description: '', type: 'APPROVAL', isBlocking: false });
      window.dispatchEvent(new CustomEvent('nexus-toast', { detail: { message: 'המשימה נוצרה בהצלחה!', type: 'success' } }));
  };
  ...
  return (
    <div className="space-y-10 animate-slide-up pb-20">
        ...
        {/* Business Owner Motivation / Success Tip */}
                    ...
                    {isLoadingRec ? (
                        ...
                        <>
                            ...
                            <div className="mt-auto flex items-center gap-3 bg-white/5 border border-white/10 p-4 rounded-2xl">
                                <div className="p-2 bg-nexus-accent/20 rounded-lg"><TrendingUp size={16} className="text-nexus-accent" /></div>
                                <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div key={goal.id} className="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-md transition-all group">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">יעד סופי: {goal.metricTarget}{goal.unit}</span>
                                </div>
            ...
            {/* 2. TASK MANAGEMENT (The Friction Reducer) */}
                ...
                <div className="bg-gray-100/50 rounded-[40px] p-6 space-y-4 min-h-[300px]">
                    {showTaskForm && (
                        <div className="bg-white border-2 border-nexus-accent rounded-3xl p-5 shadow-2xl animate-slide-up space-y-5">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => setShowTaskForm(false)} className="flex-1 py-3 text-xs font-bold text-gray-400 hover:text-gray-600 transition-colors">ביטול</button>
                                <button onClick={handleAddTask} className="flex-1 py-3 bg-nexus-primary text-white rounded-xl text-xs font-bold shadow-lg hover:bg-nexus-accent transition-all">הזרק לפורטל</button>
                            </div>
                        </div>
                    )}
                    ...
                    {client.pendingActions.filter(a => a.status === 'PENDING').map(action => (
                        <div key={action.id} className="bg-white p-5 rounded-3xl border border-gray-200 flex items-center justify-between group shadow-sm hover:border-nexus-accent transition-all">
                            <div className="flex items-center gap-4">
                                <div className={`p-3 rounded-2xl ${action.isBlocking ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400'} group-hover:scale-110 transition-transform`}>
                    ...
                    {client.pendingActions.filter(a => a.status === 'PENDING').length === 0 && !showTaskForm && (
                        <div className="h-64 flex flex-col items-center justify-center text-gray-300 italic text-sm text-center">
                            ...
                            <p>אין משימות פתוחות ללקוח בפורטל.<br/>זה זמן טוב להוסיף משהו!</p>
                        </div>
                    )}
                </div>
            </div>

            {/* 3. VAULT MANAGEMENT (The Digital Assets) */}
            <div className="lg:col-span-12 space-y-6">
                ...
                <div className="bg-white border border-gray-200 rounded-[32px] overflow-hidden shadow-sm">
                                ...
                                <tr key={asset.id} className="hover:bg-gray-50 transition-colors group">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 bg-gray-100 rounded-lg text-gray-400"><FileText size={18} /></div>
                                    ...
                                    <td className="px-8 py-5">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${asset.uploadedBy === 'AGENCY' ? 'bg-nexus-accent/10 text-nexus-accent' : 'bg-blue-50 text-blue-500'}`}>
                                            {asset.uploadedBy === 'AGENCY' ? 'הסוכנות' : 'הלקוח'}
                                        </span>
                                    </td>
                                    <td className="px-8 py-5 text-xs text-gray-400 font-mono">{asset.date}</td>
                                    <td className="px-8 py-5 text-left">
                                        <button className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {client.assets.length === 0 && (
                        <div className="py-20 text-center text-gray-400 italic">הכספת ריקה</div>
                    )}
                </div>
            </div>
        </div>
...
export default PortalManagementTab;
