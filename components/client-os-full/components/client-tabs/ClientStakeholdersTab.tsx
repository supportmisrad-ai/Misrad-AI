...
export const ClientStakeholdersTab: React.FC<ClientStakeholdersTabProps> = ({ client }) => {
  const getStakeholderIcon = (role: StakeholderRole) => {
    switch (role) {
        case 'CHAMPION': return <Crown size={16} className="text-[color:var(--os-accent)]" />;
                ...
                <div key={sh.id} className="glass-card p-0 rounded-xl border border-gray-200 overflow-hidden group hover:shadow-lg transition-all">
                    <div className="h-20 bg-gradient-to-r from-gray-100 to-gray-50 relative">
                        <div className="absolute top-3 right-3">
                            <span className={`text-[10px] font-bold px-2 py-1 rounded-full border bg-white flex items-center gap-1 ${
                                sh.nexusRole === 'CHAMPION' ? 'text-green-600 border-green-200' :
                                sh.nexusRole === 'BLOCKER' ? 'text-red-600 border-red-200' :
                                sh.nexusRole === 'DECISION_MAKER' ? 'text-blue-600 border-blue-200' :
                    ...
                    <div className="px-6 pb-6 relative">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full ${sh.sentiment > 70 ? 'bg-green-500' : sh.sentiment < 40 ? 'bg-red-500' : 'bg-[color:var(--os-accent)]'}`} style={{ width: `${sh.sentiment}%` }}></div>
                                </div>
                            </div>
                        </div>
