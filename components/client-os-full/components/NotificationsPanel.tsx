...
const NotificationsPanel: React.FC<NotificationsPanelProps> = ({ isOpen, onClose, notifications, onMarkAsRead, onDismiss, onClearAll }) => {
  ...
  const getStyle = (type: NotificationType) => {
    switch (type) {
      case 'ALERT': return 'bg-signal-danger shadow-lg shadow-signal-danger/20';
      case 'SUCCESS': return 'bg-signal-success shadow-lg shadow-signal-success/20';
      case 'MESSAGE': return 'bg-blue-500 shadow-lg shadow-blue-500/20';
      case 'TASK': return 'bg-nexus-accent shadow-lg shadow-nexus-accent/20';
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      {/* Backdrop */}
      <div 
        className="absolute inset-0 bg-nexus-primary/20 backdrop-blur-sm transition-opacity" 
        onClick={onClose}
      ></div>
      ...
      <div className="w-full md:max-w-md h-full bg-white shadow-2xl animate-slide-in-right relative flex flex-col md:border-l border-gray-100">
        ...
        <div className="p-4 md:p-6 border-b border-gray-100 bg-white/95 backdrop-blur-md z-10 pt-safe-top">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={onClose} className="p-3 md:p-2 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors active:scale-95">
          ...
          <div className="flex gap-2 items-center overflow-x-auto no-scrollbar pb-1">
              <button 
                ...
                className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all whitespace-nowrap ${activeTab === 'ALL' ? 'bg-nexus-primary text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
              >
                  הכל
              </button>
              <button 
                onClick={() => setActiveTab('UNREAD')}
                className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all whitespace-nowrap ${activeTab === 'UNREAD' ? 'bg-nexus-primary text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
              >
                  לא נקראו
              </button>
            ...
            {filteredNotifications.length > 0 ? filteredNotifications.map(notification => (
                <div 
                    ...
                    className={`p-4 rounded-xl border transition-all active:scale-[0.98] group relative
                        ${notification.isRead ? 'bg-white border-gray-100' : 'bg-white border-l-4 border-l-nexus-accent border-y-white border-r-white shadow-sm'}
                    `}
                    onClick={() => onMarkAsRead(notification.id)}
                >
                    <div className="flex gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${getStyle(notification.type)}`}>
                            {getIcon(notification.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start">
                                <h4 className={`text-sm font-bold truncate ${notification.isRead ? 'text-gray-700' : 'text-gray-900'}`}>{notification.title}</h4>
                                <span className="text-[10px] text-gray-400 whitespace-nowrap flex-shrink-0 mr-2">{notification.timestamp}</span>
                            </div>
                    ...
                    {/* Hover Actions - Visible on mobile via specific tap usually, but here we just show delete */}
