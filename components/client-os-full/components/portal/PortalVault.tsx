...
import { Zap, ClipboardList, Upload, FileSignature, RefreshCw, Check, CircleCheckBig, Image, FileText, Download } from 'lucide-react';
...
export const PortalVault: React.FC<PortalVaultProps> = ({
  pendingTasks, clientAssets, isUploading, onUpload, onOpenForm, onActionComplete
}) => {
  return (
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-4">
            {pendingTasks.map(action => (
              <div key={action.id} className="p-6 rounded-[32px] border bg-white border-slate-200 hover:border-slate-400 group shadow-sm transition-all">
                <div className="flex items-start gap-4 mb-6">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-50 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all">
                    {action.type === 'FORM' ? <ClipboardList size={24} /> : action.type === 'UPLOAD' ? <Upload size={24} /> : <FileSignature size={24} />}
                  </div>
            ...
            {clientAssets?.map(asset => (
              <div key={asset.id} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-all group">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-slate-50 rounded-xl text-slate-400 group-hover:text-nexus-primary transition-colors"><FileText size={20}/></div>
                  <div>
                    <span className="text-sm font-bold text-slate-700 block">{asset.name}</span>
                    <span className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{asset.category} • {asset.date}</span>
                  </div>
                </div>
                <button className="p-2 text-slate-300 hover:text-nexus-primary transition-colors"><Download size={20} /></button>
              </div>
            ))}
          </div>
        </div>
