...
import { Sparkles, Target, ArrowLeft, PenTool, FileSignature, CircleCheckBig, ThumbsUp, RefreshCw, ThumbsDown, MessageSquarePlus, Check } from 'lucide-react';
...
export const PortalDashboard: React.FC<PortalDashboardProps> = ({
  client, pendingTasks, approvals, northStarGoal, showTestimonialCard, 
  onCloseTestimonial, onActionComplete, onNavigate,
  selectedMood, setSelectedMood, moodComment, setMoodComment, 
  isSubmittingMood, moodSubmitted, onSubmitPulse
}) => {
  return (
    <div className="animate-slide-up space-y-10 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {northStarGoal && (
          <div onClick={() => onNavigate('metrics')} className="md:col-span-2 bg-white rounded-[40px] p-8 border border-slate-200 shadow-xl relative overflow-hidden group cursor-pointer hover:border-nexus-accent/50 transition-all">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
              <div className="w-32 h-32 flex-shrink-0 relative">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={364.4} strokeDashoffset={364.4 * (1 - northStarGoal.metricCurrent / northStarGoal.metricTarget)} className="text-nexus-accent transition-all duration-1000 ease-out" />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-black text-slate-900">{Math.round((northStarGoal.metricCurrent / northStarGoal.metricTarget) * 100)}%</span>
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex items-center gap-4">
                  <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
                    ...
                    <span className="text-xl font-bold text-slate-900">{northStarGoal.metricCurrent}{northStarGoal.unit}</span>
                  </div>
                  ...
                  <div className="bg-nexus-accent/10 px-4 py-2 rounded-2xl border border-nexus-accent/20">
                    ...
                    <span className="text-xl font-bold text-nexus-primary">{northStarGoal.metricTarget}{northStarGoal.unit}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm">
          ...
          <div className="space-y-4">
            {approvals.length > 0 ? approvals.map((action) => (
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm flex flex-col transition-all duration-500">
          <div className="flex justify-between items-center mb-6">
            ...
            {selectedMood && !moodSubmitted && (
              <button 
                onClick={() => { setSelectedMood(null); setMoodComment(''); }}
