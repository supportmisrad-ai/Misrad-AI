        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            {!hideHeader ? (
                <div className="mb-8">
                    ...
                    <p className="text-slate-600 text-base md:text-lg">בחר איזו גרסה של המערכת כל לקוח משתמש, ועדכן לפי הצורך.</p>
                </div>
            ) : null}
                ...
                <div className="md:hidden p-4 space-y-3">
                    {filteredTenants.length === 0 ? (
                        ...
                        filteredTenants.map((tenant, index) => {
                            ...
                            const selected = selectedVersion[tenant.id] || currentVersion;
                                ...
                                <div key={`${tenant.id}-${index}`} className="bg-white/80 border border-slate-200 rounded-2xl p-4">
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                                            <div className="text-slate-500 font-black">גרסה נוכחית</div>
                                            <div className={`mt-1 inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-black border ${versionClass}`}>{currentVersion}</div>
                                        </div>
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                                    ...
                                    <Button
                                        ...
                                        disabled={isUpdating || selected === currentVersion}
                                        className="mt-3 w-full bg-indigo-600 text-white font-black hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {isUpdating ? (
                                            <>
                                                <RefreshCw size={16} className="opacity-80" /> מעדכן...
                                            </>
                                        ) : (
                                            <>
                                                <Download size={16} /> עדכן
                                            </>
                                        )}
                                    </Button>
                                </div>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex gap-2">
                                                <Button
                                                    ...
                                                    size="sm"
                                                    className="bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30"
                                                >
                                                    {isUpdating ? (
                                                        <>
