        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            <div className="flex justify-between items-end mb-10">
                <div>
                    ...
                    <p className="text-slate-400 text-lg">מבט על העסקים שמשתמשים בפלטפורמה שלך.</p>
                </div>
                <button 
                    onClick={onAddClick}
                    className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-500 hover:to-purple-500 px-6 py-3.5 rounded-xl font-bold flex items-center gap-2 shadow-xl shadow-indigo-900/50 transition-all hover:scale-105 backdrop-blur-sm border border-white/10"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                ...
                <div className="bg-black/20 backdrop-blur-2xl border border-white/10 p-6 rounded-2xl shadow-xl hover:border-white/20 transition-all">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="p-3 bg-purple-500/20 text-purple-400 rounded-xl border border-purple-500/30 backdrop-blur-sm">
                            <Users size={20} />
                        </div>
                    </div>
                ...
                <table className="w-full text-right text-sm">
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300">
                        {uniqueFilteredTenants.map((tenant, index) => (
                            <tr key={`${tenant.id}-${index}`} className="hover:bg-black/30 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex -space-x-2 space-x-reverse cursor-pointer" onClick={() => onEditModules(tenant)} title="ערוך מודולים">
                                        {tenant.modules.slice(0, 3).map(mod => {
                                            ...
                                            if (!conf) return null;
                                            return (
                                                <div key={mod} className={`w-7 h-7 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 flex items-center justify-center text-xs font-bold ${conf.color}`}>
                                                    <conf.icon size={12} />
                                                </div>
                                            );
                                        })}
                                        {tenant.modules.length > 3 && (
                                            <div className="w-7 h-7 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 flex items-center justify-center text-[10px] text-slate-400">
                                                +{tenant.modules.length - 3}
                                            </div>
                                        )}
                                    </div>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            ...
                                            className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm border border-white/10 hover:bg-purple-600/80 hover:border-purple-500/50 text-white transition-all hover:scale-105" 
                                        ...
                                        {tenant.ownerEmail && (
                                            <button 
                                                ...
                                                className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm border border-white/10 hover:bg-emerald-600/80 hover:border-emerald-500/50 text-white transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed" 
                                        ...
                                        <button 
                                            ...
                                            className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm border border-white/10 hover:bg-indigo-600/80 hover:border-indigo-500/50 text-white transition-all hover:scale-105" 
                                        ...
                                        <button 
                                            ...
                                            className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm border border-white/10 hover:bg-white/10 text-white transition-all hover:scale-105" 
                                            title="ערוך פיצ׳רים"
                                        >
                                            <Settings size={14} />
                                        </button>
