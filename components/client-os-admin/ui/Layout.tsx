...
import { MOCK_NOTIFICATIONS } from '@/components/client-portal/constants';
...
const Layout: React.FC<LayoutProps> = ({ children, activeView, onNavigate }) => {
  ...
  const allNavItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'ראשי', module: null },
    { id: 'clients', icon: Users, label: 'לקוחות', module: null },
    { id: 'cycles', icon: Layers, label: 'מחזורים', module: 'cycles' },
    { id: 'email', icon: Mail, label: 'דואר', module: null },
  ...
  const navItems = allNavItems.filter((item) => !item.module || modules[item.module as keyof typeof modules]);

  const mobileExtraItems = [
    { id: 'cycles', icon: Layers, label: 'מחזורים', desc: 'ניהול קבוצות לקוחות', module: 'cycles' },
    { id: 'email', icon: Mail, label: 'דואר', desc: 'מרכז תקשורת ו-AI', module: null },
    { id: 'workflows', icon: GitMerge, label: 'תהליכים', desc: 'בניית שיטות עבודה', module: 'workflows' },
    { id: 'forms', icon: ClipboardList, label: 'טפסים', desc: 'שאלונים וקליטת לקוח', module: 'portals' },
    { id: 'feedback', icon: MessageSquareQuote, label: 'משובים', desc: 'סקרים ו-NPS', module: 'feedback' },
    { id: 'analyzer', icon: Sparkles, label: 'ניתוח טקסט', desc: 'בדיקת שיחות חופשית', module: 'intelligence' },
    { id: 'settings', icon: Settings, label: 'הגדרות', desc: 'ניהול המערכת', module: null },
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden text-nexus-primary font-sans bg-nexus-bg">
      <aside
        className="hidden lg:flex w-72 flex-shrink-0 bg-nexus-primary flex-col justify-between py-8 z-20 shadow-2xl relative overflow-hidden pt-safe pl-safe custom-scrollbar"
        tabIndex={0}
      ...
      >
        <div
          ...
          style={{ backgroundImage: 'radial-gradient(#C5A572 1px, transparent 1px)', backgroundSize: '24px 24px' }}
        ></div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 group relative border
                    ${isActive ? 'bg-white/10 text-white shadow-lg backdrop-blur-md border-white/5' : 'border-transparent text-gray-400 hover:text-white hover:bg-white/5'}`}
                >
                  {isActive && (
                  ...
                  <Icon
                    size={20}
                    className={`relative z-10 transition-colors ${isActive ? 'text-nexus-accent' : 'text-gray-500 group-hover:text-gray-300'}`}
                  />
          ...
          <div className="px-4 mt-auto relative z-10 flex-shrink-0 pb-safe">
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 border
                ${activeView === 'settings' ? 'bg-white/10 text-white border-white/5 shadow-lg backdrop-blur-md' : 'border-transparent text-gray-500 hover:text-white hover:bg-white/5'}`}
            >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-nexus-bg">
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            >
              <div className="text-right hidden lg:block">
                <div className="text-sm font-bold text-nexus-primary group-hover:text-nexus-accent transition-colors">John Doe</div>
                <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">מנהל</div>
              </div>
        ...
        <main className="flex-1 overflow-y-auto relative custom-scrollbar scroll-fade-v pb-24 lg:pb-0" role="main" tabIndex={0}>
          <div className="absolute top-0 left-0 right-0 h-64 bg-gradient-to-b from-white/40 to-transparent pointer-events-none"></div>
          <div className="p-4 lg:p-10 pt-2 lg:pt-4 h-full max-w-[1800px] mx-auto relative z-10">{children}</div>
        </main>
      </div>
        ...
        {mobileNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-bold ${isActive ? 'text-nexus-primary' : 'text-gray-400'}`}>{item.label}</span>
            </button>
          );
        })}
      </nav>
      ...
      {isMobileMenuOpen && (
        <div
          className="lg:hidden fixed inset-0 z-[60] bg-nexus-bg animate-fade-in flex flex-col no-scrollbar overflow-y-auto pt-safe pb-safe"
          role="dialog"
          aria-modal="true"
          aria-label="Full menu"
        >
          <div className="sticky top-0 bg-nexus-bg/90 backdrop-blur-md p-6 flex justify-between items-center border-b border-gray-200/50 z-10">
            <div>
