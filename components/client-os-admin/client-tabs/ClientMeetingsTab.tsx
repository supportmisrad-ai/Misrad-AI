...
import { Meeting } from '@/components/client-portal/types';
import {
  Calendar,
  Video,
  Users,
            ...
            <div
              ...
              className={`bg-white border rounded-xl transition-all overflow-hidden ${
                expandedMeetingId === meeting.id
                  ? 'border-nexus-primary shadow-lg ring-1 ring-nexus-primary/20'
                  : 'border-gray-200 hover:border-nexus-primary/30'
              }`}
            >
              <div className="p-5 cursor-pointer flex justify-between items-start" onClick={() => onToggleExpand(meeting.id)}>
                <div className="flex items-center gap-4">
                  <div
                    className={`p-3 rounded-xl flex flex-col items-center justify-center w-14 h-14 ${
                      expandedMeetingId === meeting.id ? 'bg-nexus-primary text-white' : 'bg-gray-100 text-gray-600'
                    }`}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                      <span className="flex items-center gap-1">
                        {meeting.location === 'ZOOM' ? <Video size={12} /> : <Users size={12} />} {meeting.location}
                      </span>
                      <span>•</span>
                  ...
                  {meeting.aiAnalysis && (
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded border ${
                        meeting.aiAnalysis.sentimentScore > 70
                      ...
                      {meeting.aiAnalysis.liabilityRisks.length > 0 && (
                        <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                          <h5 className="text-xs font-bold text-red-600 uppercase tracking-widest mb-2 flex items-center gap-1">
                            <ShieldAlert size={12} /> סיכונים והבטחות
                          </h5>
                      ...
                      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div className="space-y-2">
                          ...
                          {meeting.files?.map((file, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer group"
                            >
                              <div className="p-1.5 bg-white rounded shadow-sm text-gray-500 group-hover:text-nexus-primary">
                                <FileText size={16} />
                              </div>
                            ...
                            <button
                              ...
                              className={`w-full text-right flex items-start gap-3 p-3 rounded-xl border transition-all ${
                                task.status === 'COMPLETED'
                                  ...
                                  : 'bg-white border-gray-200 hover:border-nexus-primary/30'
                              }`}
                            >
                              <div
                                className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                                  task.status === 'COMPLETED'
                                    ...
                                    : 'border-gray-300 text-transparent hover:border-nexus-primary'
                                }`}
                              >
                                <Check size={14} />
                              </div>
                              <div>
                                <span className={`text-sm font-medium block ${task.status === 'COMPLETED' ? 'line-through text-gray-500' : 'text-gray-800'}`}>{task.task}</span>
