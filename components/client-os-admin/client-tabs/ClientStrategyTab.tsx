...
import { Client, Opportunity } from '@/components/client-portal/types';
import { Trophy, Check, TrendingUp, Plus, Zap, Target, CreditCard, Gauge } from 'lucide-react';
...
export const ClientStrategyTab: React.FC<ClientStrategyTabProps> = ({ client, opportunities, onAddOpportunity }) => {
  ...
  return (
    <div className="space-y-8 animate-slide-up">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{goal.deadline}</span>
                    </div>
                  </div>
                  <span
                  ...
                  >
                    {goal.status === 'ACHIEVED' ? 'הושלם' : goal.status === 'AT_RISK' ? 'בסיכון' : 'בתהליך'}
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold uppercase">מצב נוכחי</span>
                    ...
                    <div className="flex flex-col text-left">
                      <span className="text-[10px] text-gray-400 font-bold uppercase">יעד</span>
                      <span className="text-xl font-mono font-black text-nexus-accent">
                        {goal.metricTarget}
                        {goal.unit}
                      </span>
                    </div>
      ...
      <div className="glass-card p-0 rounded-2xl overflow-hidden flex flex-col md:flex-row">
        <div className={`p-8 flex-1 relative overflow-hidden text-white ${totalROI >= 0 ? 'bg-nexus-primary' : 'bg-red-900'}`}>
          ...
          <div className="relative z-10">
            <h3 className="text-nexus-accent font-bold uppercase tracking-widest text-xs mb-4 flex items-center gap-2">
              <Trophy size={14} /> {totalROI >= 0 ? 'כמה הרוויחו מאיתנו (ROI)' : 'מאזן ROI שלילי'}
            </h3>
            <div className="text-5xl font-display font-bold mb-2">₪{totalROI.toLocaleString()}</div>
            <p className="text-gray-400 text-sm">סה"כ הצלחות פחות החזרים וזיכויים</p>
          </div>
        </div>
        <div className="p-6 bg-white flex-1">
          <h4 className="text-gray-900 font-bold text-sm mb-4">היסטוריית הצלחות ואירועים</h4>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            <TrendingUp size={20} className="text-nexus-primary" /> הזדמנויות צמיחה
          </h3>
          <button
            onClick={onAddOpportunity}
            className="px-4 py-2 bg-nexus-accent text-white rounded-xl text-xs font-bold hover:bg-nexus-accent/90 transition-colors flex items-center gap-2"
          >
            <Plus size={16} /> הוסף הצעה
          </button>
        </div>
          ...
          {opportunities.map((opp) => (
            <div key={opp.id} className="glass-card p-5 rounded-xl border hover:border-nexus-accent/40 transition-all group">
              <div className="flex justify-between items-start mb-3">
                <div className="p-2 bg-nexus-accent/10 rounded-lg text-nexus-accent">
                  <Zap size={18} />
                </div>
              ...
              <div className="text-lg font-mono font-bold text-nexus-primary">₪{opp.value.toLocaleString()}</div>
            </div>
          ))}
        </div>
      </div>
