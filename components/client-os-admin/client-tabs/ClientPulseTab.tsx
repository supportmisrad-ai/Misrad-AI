...
export const ClientPulseTab: React.FC<ClientPulseTabProps> = ({ client, aiInsight, isInsightLoading, onGenerateInsight }) => {
  const getDnaData = (c: Client) => {
    const roiScore = Math.min(100, c.roiHistory.length * 25);
    return [
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass-card p-6 rounded-2xl flex flex-col items-center justify-center relative min-h-[400px] bg-white/90">
          <div className="absolute top-4 left-4 text-xs font-bold bg-gray-100 px-3 py-1 rounded-full border border-gray-200">ציון: {client.healthScore}</div>
          <ResponsiveContainer width="100%" height={350}>
            <RadarChart cx="50%" cy="50%" outerRadius="80%" data={getDnaData(client)}>
              <PolarGrid stroke="#e5e7eb" />
              <PolarAngleAxis dataKey="subject" tick={{ fill: '#64748B', fontSize: 12, fontWeight: 'bold' }} />
              <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
              <Radar name="Client Health" dataKey="A" stroke="#0F172A" strokeWidth={3} fill="#C5A572" fillOpacity={0.4} />
            </RadarChart>
          </ResponsiveContainer>
        ...
        <div className="flex flex-col gap-6">
          <div className="glass-card p-6 rounded-2xl border-l-4 border-l-nexus-accent bg-white relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              {aiInsight ? (
                <>
                  <p className="text-sm text-gray-600 leading-relaxed font-medium">"{aiInsight.insight}"</p>
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                    ...
                    <button className="mr-auto text-xs font-bold text-nexus-accent hover:underline">בצע</button>
                  </div>
                </>
              ) : (
                ...
                .map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-red-50/50 border border-red-100">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-red-500 font-bold text-xs shadow-sm">{item.A}</div>
                      <div>
