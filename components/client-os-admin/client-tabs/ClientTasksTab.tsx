...
import { Client, AssignedForm, AutomationSequence, ScheduledAutomation } from '@/components/client-portal/types';
...
export const ClientTasksTab: React.FC<ClientTasksTabProps> = ({ client, assignedForms, onAssignForm, activeSequences, scheduledAutomations }) => {
  return (
        ...
        <div className="lg:col-span-2 space-y-8">
                  ...
                  <div key={action.id} className="bg-red-50 border border-red-100 p-4 rounded-xl flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-[10px] font-bold bg-white px-2 py-0.5 rounded text-red-500 border border-red-200">{action.dueDate}</span>
                        {action.isBlocking && <span className="text-[10px] font-bold text-red-600">חוסם התקדמות!</span>}
                      </div>
                    </div>
                    <button className="text-xs font-bold text-red-600 hover:underline bg-white px-2 py-1 rounded border border-red-200">שלח תזכורת</button>
                  </div>
                ))
              ) : (
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={onAssignForm} className="text-xs font-bold text-nexus-accent hover:underline flex items-center gap-1">
                ...
                <div key={form.id} className="bg-white border border-gray-200 p-4 rounded-xl flex items-center gap-4 hover:border-nexus-primary/30 transition-all">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between mt-2 text-[10px] text-gray-400">
                      ...
                      <span className={form.status === 'COMPLETED' ? 'text-green-600 font-bold' : ''}>{form.status === 'COMPLETED' ? 'הושלם' : 'בתהליך'}</span>
                    </div>
                  </div>
                </div>
              ))}
              {assignedForms.length === 0 && (
                <div className="text-center py-6 text-gray-400 text-sm border-2 border-dashed border-gray-100 rounded-xl">לא נשלחו טפסים</div>
              )}
            </div>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {scheduledAutomations.length === 0 && (
                      <div className="text-center py-4 text-white/30 text-xs italic border border-dashed border-white/10 rounded-xl">התור ריק</div>
                    )}
                  </div>
                </div>
