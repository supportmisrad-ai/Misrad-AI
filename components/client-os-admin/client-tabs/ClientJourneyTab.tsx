...
import { JourneyStage } from '@/components/client-portal/types';
        ...
        {journeyData?.map((stage) => {
          ...
          return (
            <div key={stage.id} className="relative pr-8">
              <div
                className={`absolute top-0 right-[-9px] w-4 h-4 rounded-full border-2 border-white shadow-sm z-10 ${
                  isActive ? 'bg-nexus-accent animate-pulse' : isCompleted ? 'bg-nexus-primary' : 'bg-gray-300'
                }`}
              ></div>

              <div className="flex items-center justify-between mb-2">
                <h4 className={`text-lg font-bold ${isActive ? 'text-nexus-accent' : isCompleted ? 'text-gray-900' : 'text-gray-400'}`}>{stage.name}</h4>
                <span className="text-xs text-gray-500 font-mono">{stage.date}</span>
              </div>

              <div
                className={`bg-white border rounded-xl overflow-hidden transition-all duration-300 ${
                  isActive ? 'border-nexus-accent/30 shadow-md' : 'border-gray-200'
                }`}
              >
                {stage.milestones?.map((m) => (
                  <div key={m.id} className="p-3 border-b border-gray-100 last:border-0 flex items-center gap-3">
                    ...
                    <span className={`text-sm ${m.isCompleted ? 'text-gray-900 line-through opacity-60' : 'text-gray-800 font-medium'}`}>{m.label}</span>
                  </div>
                ))}
