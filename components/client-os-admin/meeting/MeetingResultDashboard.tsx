...
export const MeetingResultDashboard: React.FC<MeetingResultDashboardProps> = ({
  ...
  const getRiskColor = (level: string) => {
    switch (level) {
      case 'HIGH':
        return 'text-signal-danger border-signal-danger/30 bg-signal-danger/10';
      case 'MEDIUM':
        return 'text-signal-warning border-signal-warning/30 bg-signal-warning/10';
      default:
  ...
  return (
    <div className="animate-slide-up space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-nexus-primary transition-colors w-fit">
          <ArrowLeft size={18} /> חזרה לרשימה
        </button>
        <div className="flex gap-2 w-full md:w-auto">
          <button className="flex-1 md:flex-none px-4 py-3 md:py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 hover:bg-gray-50 whitespace-nowrap">
            ייצא PDF
          </button>
          <button className="flex-1 md:flex-none px-4 py-3 md:py-2 bg-nexus-primary text-white rounded-xl text-xs font-bold hover:bg-nexus-accent whitespace-nowrap">
            שתף סיכום
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card p-4 md:p-6 rounded-2xl border-l-4 border-l-nexus-primary relative overflow-hidden flex flex-col gap-6">
          ...
          <div>
            ...
            <p className="text-gray-700 leading-relaxed text-sm bg-gray-50 p-4 rounded-xl border border-gray-100">{analysisResult?.summary}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="glass-card p-4 rounded-xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="w-16 md:w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="bg-blue-600 h-full" style={{ width: `${analysisResult?.rating?.professionalism}%` }}></div>
              </div>
          ...
          <div className="glass-card p-4 rounded-xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="w-16 md:w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="bg-pink-600 h-full" style={{ width: `${analysisResult?.rating?.warmth}%` }}></div>
              </div>
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <button
              ...
              className={`p-4 rounded-xl border transition-all text-right flex flex-col gap-3 group relative overflow-hidden active:scale-[0.98] ${
                activeAutomations.includes('SUMMARY') ? 'bg-white text-nexus-primary border-white' : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}
            >
            ...
            <button
              ...
              className={`p-4 rounded-xl border transition-all text-right flex flex-col gap-3 group relative overflow-hidden active:scale-[0.98] ${
                activeAutomations.includes('TASKS') ? 'bg-white text-nexus-primary border-white' : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}
            >
              <div className="flex justify-between items-start w-full">
            ...
            <button
              ...
              className={`p-4 rounded-xl border transition-all text-right flex flex-col gap-3 group relative overflow-hidden active:scale-[0.98] ${
                activeAutomations.includes('MEETING') ? 'bg-white text-nexus-primary border-white' : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}
            >
              <div className="flex justify-between items-start w-full">
      ...
      <div className="glass-card p-6 rounded-2xl">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-nexus-accent/10 text-nexus-accent rounded-xl">
