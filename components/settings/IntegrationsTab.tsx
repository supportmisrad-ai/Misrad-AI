...
import { Zap, Copy, Check, Play, FileText, TriangleAlert, Video, CircleCheck } from 'lucide-react';
...
export const IntegrationsTab: React.FC = () => {
    ...
    return (
        <motion.div key="integrations" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8 pb-16 md:pb-20 px-4 sm:px-6 lg:px-8">
            <AnimatePresence>
            ...
            <div>
                <h2 className="text-xl font-bold text-gray-900">חיבורים אישיים</h2>
                <p className="text-sm text-gray-500">חבר את החשבונות האישיים שלך ל-MISRAD AI.</p>
            </div>
            ...
            {/* Link to System Settings for Sales Integrations */}
            <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">הגדרות Voicenter, Telephony ו-CRM Webhook זמינות בהגדרות מודול המכירות.</p>
                    </div>
                    <a 
                        href="/w/system/settings" 
                        className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-bold transition-colors inline-flex items-center gap-2 whitespace-nowrap"
                    >
                        עבור להגדרות מכירות <span aria-hidden="true">←</span>
                    </a>
                </div>
            </div>
        </motion.div>
