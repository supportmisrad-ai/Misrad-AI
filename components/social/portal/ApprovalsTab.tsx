...
import { CheckCircle2, X, ImageIcon } from 'lucide-react';
      ...
      {pendingPosts.length > 0 ? pendingPosts.map(post => (
        <div key={post.id} className="bg-white rounded-[48px] border border-slate-200 shadow-xl overflow-hidden flex flex-col">
          <div className="aspect-[4/3] bg-slate-100 overflow-hidden relative group">
            ...
            {!post.mediaUrl && <div className="w-full h-full flex items-center justify-center text-slate-300"><ImageIcon size={48}/></div>}
          </div>
          <div className="p-10 flex flex-col gap-6">
            <p className="font-bold text-slate-700 italic text-lg leading-relaxed">"{post.content}"</p>
            <div className="flex gap-4">
              <button onClick={() => onApprove(post.id)} className="flex-1 bg-green-500 text-white py-4 rounded-2xl font-black shadow-lg flex items-center justify-center gap-2"><CheckCircle2 size={20}/> אשר</button>
        ...
        <div className="col-span-full py-20 bg-white rounded-[56px] border-2 border-dashed flex flex-col items-center justify-center gap-6 text-center">
          <div className="w-24 h-24 bg-green-50 text-green-500 rounded-full flex items-center justify-center"><CheckCircle2 size={48}/></div>
