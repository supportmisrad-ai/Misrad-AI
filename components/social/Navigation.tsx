...
const DEFAULT_ITEMS: SocialNavigationItem[] = [
  ...
  { id: 'content-bank', label: 'בנק תכנים', icon: 'Database', view: 'content-bank', section: 'global', order: 5, isVisible: true },
    ...
    return (
      <Link
        ...
        className={`
          ...
          ${isActive
            ? 'bg-gradient-to-l from-purple-50 to-violet-50 text-slate-900 shadow-sm border border-purple-100'
      ...
      >
        ...
        {isSidebarOpen && (
          <>
            ...
            {isActive && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-purple-500 to-violet-500 rounded-r-full" />
            )}
          </>
        )}
      </Link>
