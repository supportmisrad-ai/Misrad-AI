...
const MarketingView: React.FC<MarketingViewProps> = ({ 
  ...
  const toggleStatus = (id: string) => {
      if (!canAccess('delete_campaign') || !onUpdateCampaign) return;
  ...
  const handleDelete = (id: string) => {
      if (!canAccess('delete_campaign') || !onDeleteCampaign) return;
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-8 py-1 sticky top-0 z-30 shadow-sm md:hidden">
            <div className="max-w-[1920px] mx-auto flex gap-6 overflow-x-auto no-scrollbar">
                {[
                    ...
                    { id: 'partners', label: 'שתפ"ים', icon: Network },
                ].map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveSubTab(tab.id as 'content' | 'forms' | 'campaigns' | 'partners')}
