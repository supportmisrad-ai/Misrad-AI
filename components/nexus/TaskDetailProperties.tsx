...
import { Activity, AlertTriangle, User as UserIcon, Calendar, Briefcase, Timer, Clock, Edit2, Info, ChevronDown, Sparkles } from 'lucide-react';
...
export const TaskDetailProperties: React.FC<TaskDetailPropertiesProps> = ({ task, onOpenPopover, activePopover }) => {
    const { users, updateTask, workflowStages, clients, currentUser } = useData();
    ...
    return (
        <div className="space-y-6">
            {missingFields > 0 && (
                <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl">
                    ...
                    <span className="text-xs font-bold text-amber-700">
                        {missingFields} שדות מומלצים חסרים: {!hasDueDate && 'תאריך יעד'} {!hasEstimate && 'הערכת זמן'} {!hasClient && 'לקוח'}
                    </span>
                </div>
            )}
            ...
            <div className="col-span-1 lg:col-span-1 relative">
                {!hasClient && (
                    <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full border-2 border-white animate-pulse z-10" title="מומלץ: שיוך ללקוח" />
                )}
                <RenderLabel icon={<Briefcase size={12} />} label="לקוח / פרויקט" />
                <div className={!hasClient ? 'ring-1 ring-amber-200 rounded-lg' : ''}>
                    <CustomSelect 
                        value={task.clientId || ''}
            ...
            <div className="col-span-2 lg:col-span-1 flex gap-2">
                <div className="flex-1 relative">
                    {!hasEstimate && (
                        <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full border-2 border-white animate-pulse z-10" title="מומלץ: הערכת זמן" />
                    )}
                    <RenderLabel icon={<Timer size={12} />} label="הערכה" />
                    <button
                        ref={estimateButtonRef}
                        onClick={() => handlePopoverClick('estimate', estimateButtonRef)}
                        className={`${BUTTON_CLASS} ${activePopover === 'estimate' ? ACTIVE_CLASS : ''} ${!hasEstimate ? 'ring-1 ring-amber-200' : ''}`}
                    >
