    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex justify-center items-end md:items-center animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        <div className="p-8 md:p-10 border-t border-slate-100 bg-slate-50 flex gap-4 shrink-0 safe-area-bottom">
            <button onClick={onClose} className="flex-1 bg-white border border-slate-200 text-slate-700 font-bold py-4 rounded-full hover:bg-slate-50 transition-colors">ביטול</button>
            <button type="submit" form="new-lead-form" className="flex-1 bg-onyx-900 text-white font-bold py-4 rounded-full hover:bg-black transition-all shadow-xl hover:-translate-y-1 flex items-center justify-center gap-2">
                <Save size={20} /> שמור ליד
            </button>
        </div>
      </div>
    </div>
