...
export default function SmartRoutingSuggestion({
  orgSlug,
  onAccept,
}: {
  orgSlug: string;
  ...
  async function handleSuggest() {
    ...
    try {
      // Gather form values from the DOM
      ...
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || data?.message || 'שגיאה בהמלצת טכנאי');
      }
      ...
      if (s) {
        ...
        setError(data.data?.reason || 'לא נמצאה המלצה');
      }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'שגיאה בהמלצת טכנאי');
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function handleAccept() {
    ...
    onAccept(suggestion.technicianId);
    setAccepted(true);
  }
    ...
    <div className="rounded-xl border border-violet-100 bg-violet-50/50 p-3">
      <div className="flex items-center justify-between gap-2 mb-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] font-black text-violet-700">המלצת AI</span>
        </div>
        <button
          type="button"
      ...
      {error ? (
        <div className="text-[11px] text-rose-600 font-bold mt-1">{error}</div>
      ) : null}

      {suggestion ? (
        <div className="mt-1.5 flex items-center justify-between gap-2 bg-white rounded-lg px-3 py-2 border border-violet-100">
