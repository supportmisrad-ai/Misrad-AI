            ...
            <div className="p-6 bg-white border-t border-slate-200 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)] z-30">
                {step === 'build' ? (
                    ...
                    <div className="space-y-4 animate-slide-up">
                        ...
                        <button 
                            onClick={() => window.open(`https://wa.me/${String((lead as unknown as Record<string, unknown>)?.phone ?? '').replace(/-/g, '')}?text=${encodeURIComponent(`היי ${String((lead as unknown as Record<string, unknown>)?.name ?? '').split(' ')[0]}, הנה הסכם העבודה לחתימה דיגיטלית: ${generatedLink}`)}`, '_blank')}
