      ...
      >
        ...
        <div className={`${mobileView === 'info' ? 'flex' : 'hidden lg:flex'} w-full lg:w-[25%] bg-slate-50 border-b lg:border-b-0 lg:border-l border-slate-200 flex flex-col overflow-y-auto shrink-0 relative lg:h-full`}>
            ...
            <div className="p-4 bg-white/50 border-b border-slate-200/50">
                ...
                {!String((lead as any)?.email || '').trim() && (
                    <div className="mt-2 text-[11px] font-bold text-slate-500">
                        לא ניתן לפתוח פורטל לקוח כי לליד אין אימייל.
                    </div>
                )}
            </div>
            ...
            <div className="p-4 grid grid-cols-4 gap-2 border-b border-slate-200/50">
                ...
                <button onClick={() => onScheduleMeeting(lead.id)} className="flex flex-col items-center justify-center p-2 rounded-xl bg-white text-slate-600 hover:text-indigo-600 border border-slate-200 shadow-sm"><CalendarClock size={18} /><span className="text-[9px] font-bold">מעקב</span></button>
            </div>
        </div>
