...
import { STAGES, NAV_ITEMS, NAV_GROUPS, INITIAL_LEADS, INITIAL_CAMPAIGNS, INITIAL_AGENTS, INITIAL_TASKS, INITIAL_CONTENT, INITIAL_STUDENTS, INITIAL_INVOICES } from './constants';
...
import PersonalAreaView from './components/PersonalAreaView';
import Sidebar from './components/Sidebar';
import CalendarView from '../../nexus/CalendarView';
...
const SystemOSApp = () => {
  ...
  useEffect(() => {
      if (user) {
          const label = NAV_ITEMS.find(n => n.id === activeTab)?.label || 'Sistem.OS';
  ...
  const getHeaderActions = () => {
      switch(activeTab) {
          ...
          case 'tasks':
              return (
                <button onClick={() => setStoredTasks(p => [{ id: `t_${Date.now()}`, title: 'משימה חדשה', assigneeId: user?.id || 'u1', dueDate: new Date(), priority: 'medium', status: 'todo', tags: ['כללי'] }, ...p])} className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-slate-900/20 transition-all flex items-center gap-2">
  ...
  return (
    <div className="app-layout font-sans text-onyx-900 bg-transparent">
      ...
      <main className="main-content">
          {/* Header strictly for utility tools and profile */}
          <header className="header mb-4 relative sticky top-0 z-40 bg-white/80 md:bg-transparent backdrop-blur-xl md:backdrop-blur-none px-4 md:px-0 py-4 md:py-2 flex justify-end items-center">
            
            <div className="flex items-center gap-2 md:gap-4 justify-end w-full">
                <button onClick={() => setIsCommandPaletteOpen(true)} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/60 backdrop-blur-md border border-white/50 text-slate-400 hover:text-primary transition-all shrink-0" aria-label="חפש במערכת"><Search size={18} /></button>
                <button onClick={() => setActiveTab('notifications_center')} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/60 backdrop-blur-md border border-white/50 text-slate-400 hover:text-primary transition-all relative shrink-0" aria-label="התראות"><Bell size={18} /><span className="absolute top-2.5 right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white"></span></button>

                <div className="relative hidden md:block" ref={profileRef}>
                    <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-full bg-white/60 backdrop-blur-md border border-white/50 hover:bg-white transition-all shadow-sm group ml-2" aria-label="תפריט משתמש">
                        <div className="hidden md:flex flex-col items-end">
                            <span className="text-xs font-bold text-slate-700 leading-tight">{safeUserName}</span>
          ...
          <div 
            className="workspace custom-scrollbar relative" 
            ref={workspaceRef}
            tabIndex={0}
            role="region"
          ...
          >
              <ErrorBoundary>
                  <div className="max-w-[1920px] mx-auto min-h-full pb-20 md:pb-12 px-2">
                      ...
                      <AnimatePresence mode='wait'>
                          <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="h-full">
                              ...
                              {activeTab === 'finance' && <FinanceView invoices={storedInvoices} onAddInvoice={(i) => setStoredInvoices(p => [i, ...p])} onUpdateInvoice={(i) => setStoredInvoices(p => p.map(inv => inv.id === i.id ? i : inv))} leads={leads} />}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200/60 z-50 pb-6 pt-2 px-2 transition-all duration-300 rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          <div className="flex justify-around items-center h-[60px] relative">
              ...
              <button onClick={() => setActiveTab('sales_pipeline')} className={`p-3 rounded-full transition-colors ${activeTab === 'sales_pipeline' ? 'text-primary bg-rose-50' : 'text-slate-400'}`} aria-label="מכירות"><Kanban size={24} /></button>
              ...
              <button onClick={() => setActiveTab('mkt_campaigns')} className={`p-3 rounded-full transition-colors ${activeTab === 'mkt_campaigns' ? 'text-primary bg-rose-50' : 'text-slate-400'}`} aria-label="שיווק"><Megaphone size={24} /></button>
