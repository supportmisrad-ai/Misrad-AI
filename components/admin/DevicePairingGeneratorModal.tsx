          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">פתחו בטאבלט: כניסה למסופון → סריקת QR</div>
              </div>
              <button
                onClick={generate}
                disabled={isLoading}
                className="px-3 py-2 rounded-xl border border-gray-200 text-gray-800 font-bold text-sm hover:bg-gray-50 disabled:opacity-60"
              >
                <span className="inline-flex items-center gap-2"><RefreshCcw size={16} /> חדש</span>
              </button>
            </div>

            <div className="mt-5 rounded-2xl border border-gray-200 bg-gray-50 p-4 flex items-center justify-center">
              {token ? (
                <div className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm">
                  {qrDataUrl ? (
                    <img src={qrDataUrl} alt="QR" width={220} height={220} />
                  ) : (
                    <div className="text-gray-500 font-bold flex items-center gap-2"><QrCode size={18} /> טוען QR…</div>
                  )}
                </div>
              ) : (
                <div className="text-gray-500 font-bold flex items-center gap-2"><QrCode size={18} /> טוען QR…</div>
            ...
            <div className="mt-4">
              <div className="text-xs font-bold text-gray-500 uppercase">Token</div>
              <div className="mt-1 rounded-xl border border-gray-200 bg-white px-3 py-2 font-mono text-xs text-gray-900 break-all">{token || '—'}</div>
              {expiresText ? <div className="mt-2 text-xs text-gray-500">בתוקף עד {expiresText}</div> : null}
            </div>

            <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-gray-100">
              <button
                onClick={props.onCloseAction}
                className="px-4 py-2.5 text-gray-600 hover:bg-gray-100 rounded-lg text-sm font-bold"
              >
                סגור
              </button>
            </div>
