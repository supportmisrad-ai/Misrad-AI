...
export function ModuleHelpVideos(props: { moduleKey: OSModuleKey }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen ? (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" onClick={() => setIsOpen(false)} dir="rtl">
            <motion.div
              initial={{ opacity: 0, scale: 0.96, y: 16 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: 16 }}
              className="bg-white w-full max-w-4xl rounded-[28px] shadow-2xl overflow-hidden relative"
