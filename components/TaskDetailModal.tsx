...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, onClose }) => {
  ...
  const handleOpenPopover = (type: typeof activePopover, rect: DOMRect) => {
      ...
      const coords: { top: number; left?: number; right?: number; width?: number } = {
          top: rect.bottom + 5,
  ...
  return (
    ...
    <DeleteConfirmationModal
        ...
        title="מחיקת משימה"
        description={isSafeDelete ? "המשימה תועבר לסל המיחזור." : "זוהי משימה בעדיפות גבוהה או שלא נוצרה על ידך. מחיקתה תתועד."}
        itemName={task.title}
        isHardDelete={false}
    />
    ...
    <AnimatePresence>
        {isSnoozeModalOpen && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setIsSnoozeModalOpen(false)}>
                <motion.div 
                ...
                >
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => quickSnooze(1)} className="p-3 border border-gray-200 rounded-xl hover:border-orange-300 hover:bg-orange-50 transition-all text-sm font-bold text-gray-600">
                                מחר
                            </button>
                            <button onClick={() => quickSnooze(3)} className="p-3 border border-gray-200 rounded-xl hover:border-orange-300 hover:bg-orange-50 transition-all text-sm font-bold text-gray-600">
                                בעוד 3 ימים
                            </button>
                            <button onClick={() => quickSnooze(7)} className="p-3 border border-gray-200 rounded-xl hover:border-orange-300 hover:bg-orange-50 transition-all text-sm font-bold text-gray-600">
                                שבוע הבא
                            </button>
                            <button onClick={() => quickSnooze(30)} className="p-3 border border-gray-200 rounded-xl hover:border-orange-300 hover:bg-orange-50 transition-all text-sm font-bold text-gray-600">
                                חודש הבא
                            </button>
                        </div>
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">תאריך יעד חדש</label>
                            <CustomDatePicker 
                                value={snoozeDate}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">סיבת הדחייה (אופציונלי)</label>
                            <textarea 
                                value={snoozeReason}
    ...
    <div className="fixed inset-0 bg-slate-900/70 z-[100] flex items-center justify-center p-0 md:p-6 backdrop-blur-sm" onClick={onClose}>
      <motion.div 
      ...
      >
        ...
        <div className="md:hidden bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-20">
           <div className="flex items-center gap-3 overflow-hidden w-full">
               <button onClick={onClose} className="bg-gray-50 text-gray-600 p-2 rounded-full shrink-0"><X size={20} /></button>
        ...
        <div className="md:hidden flex bg-white border-b border-gray-100 px-2 pt-2">
               <button onClick={() => setActiveTab('chat')} className={`flex-1 pb-3 text-sm font-bold transition-all border-b-2 ${activeTab === 'chat' ? 'border-black text-black' : 'border-transparent text-gray-400'}`}>הערות ועדכונים</button>
               <button onClick={() => setActiveTab('details')} className={`flex-1 pb-3 text-sm font-bold transition-all border-b-2 ${activeTab === 'details' ? 'border-black text-black' : 'border-transparent text-gray-400'}`}>פרטים ראשיים</button>
        </div>
        ...
        <div className={`w-full md:w-[65%] flex flex-col bg-white overflow-y-auto custom-scrollbar ${activeTab === 'details' ? 'flex-1 h-full min-h-0' : 'hidden md:flex h-full'}`}>
            <div className="hidden md:flex items-center justify-between p-6 pb-2 bg-white sticky top-0 z-20">
                <div className="flex items-center gap-2 text-gray-400 font-mono text-xs">
                    ...
                    <AnimatePresence>
                        {isSaving && (
                            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-1 text-green-600 font-bold ml-2">
                                <Check size={12} /> נשמר
                            </motion.span>
                        )}
                    </AnimatePresence>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-black hover:bg-gray-100 p-2 rounded-full transition-colors ml-2">
                        <X size={20} />
                    </button>
                </div>
            </div>
            ...
            <div className="p-6 md:p-8 space-y-8">
                ...
                {(task.snoozeCount || 0) > 0 && (
                    <div className={`text-xs px-3 py-2 rounded-lg font-bold flex items-center gap-2 border ${
                        (task.snoozeCount || 0) >= 3 ? 'bg-red-50 text-red-600 border-red-200' : 'bg-orange-50 text-orange-600 border-orange-200'
                    }`}>
                        <Clock size={14} /> 
                ...
                <div>
                    ...
                    <textarea 
                        className="w-full min-h-[200px] p-4 text-sm text-gray-700 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-100 outline-none resize-none transition-all leading-relaxed hover:bg-gray-100/50"
