...
import { X, Video, Sparkles, Camera, Check } from 'lucide-react';
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
        ...
        <div className="flex-1 text-right space-y-6">
          ...
          <p className="text-white/60 text-lg lg:text-xl font-medium leading-relaxed max-w-xl">
            אנחנו ממש מעריכים את השותפות איתכם. נחמד אם תוכלו להקליט סרטון המלצה קצר של 30 שניות.
          </p>
          ...
          <div className="flex flex-col sm:flex-row-reverse gap-4 pt-4">
            ...
            >
              {isGenerating ? <Skeleton className="w-5 h-5 rounded-full bg-white/20" /> : <><Sparkles size={18} className="text-[#C5A572]" /> תנסחו לי מה להגיד</>}
            </button>
          </div>
          ...
          {script && (
            <div className="mt-6 p-6 bg-white/5 border border-[#C5A572]/20 rounded-3xl text-right animate-slide-up relative group/script">
              <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={() => setScript(null)} className="text-white/20 hover:text-white">
                  <X size={14} />
                </button>
              </div>
