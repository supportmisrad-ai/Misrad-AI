          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-nexus-primary/10 rounded-xl text-nexus-primary">
                  <Target size={20} />
                </div>
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">{plan.title}</h2>
              </div>
              <p className="text-slate-500 text-sm font-medium pr-1">{plan.description}</p>
            </div>
            <div className="bg-white px-4 py-2 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-6">
              <div className="text-center border-l border-slate-50 pl-6">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">התקדמות</div>
                <div className="text-lg font-bold text-nexus-primary">
                  {Math.round((plan.phases.filter((p: { status: string }) => p.status === 'COMPLETED').length / plan.phases.length) * 100)}%
                </div>
              </div>
              <div className="text-center">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">סטטוס</div>
                <div className="flex items-center gap-1.5">
                   <span className="w-2 h-2 rounded-full bg-green-500"></span>
                   <span className="text-sm font-bold text-slate-700">פעיל</span>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex items-start gap-6 mb-8 relative z-10">
                      ...
                      <div className="pt-1">
                        <div className="flex items-center gap-3">
                          <h3 className={`font-bold text-xl tracking-tight ${isActive ? 'text-slate-900' : 'text-slate-500'}`}>
                            {phase.title}
                          </h3>
                          {isActive && (
                        ...
                        <p className="text-sm text-slate-400 font-medium mt-1">{phase.description || `שלב ${phaseIdx + 1} בתהליך הליווי המקצועי`}</p>
                      </div>
                    </div>
                    ...
                    <div className="md:mr-16 grid grid-cols-1 lg:grid-cols-12 gap-6">
                      ...
                      <div className="lg:col-span-8 space-y-3">
                        {phase.meetings.map((meeting: Meeting) => (
                          <div 
                            ...
                            className="group bg-white border border-slate-100 rounded-2xl p-5 hover:border-nexus-primary hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer flex justify-between items-center shadow-sm"
                          >
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-nexus-primary group-hover:text-white transition-all duration-300">
                                <Video size={20} />
                              </div>
                              <div>
                                <div className="font-bold text-slate-900 group-hover:text-nexus-primary transition-colors text-lg">{meeting.title}</div>
                                <div className="flex items-center gap-3 mt-1">
                                  <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
                                    <Calendar size={12} /> {meeting.date}
                                  </div>
                                  {meeting.aiAnalysis && (
                                    <div className="flex items-center gap-1 text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-md border border-green-100 uppercase tracking-tighter">
                            ...
                            <div className="flex items-center gap-2">
                               <div className="text-[10px] font-bold text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity">צפה בסיכום</div>
                               <ChevronRight size={20} className="text-slate-200 group-hover:text-nexus-primary group-hover:translate-x-1 transition-all" />
                            </div>
                          </div>
                        ))}
                        ...
                        {phase.templates
                          ...
                          <div 
                            ...
                            className={`rounded-2xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all border-2 border-dashed ${
                              isActive ? 'bg-nexus-primary/5 border-nexus-primary/20 ring-1 ring-nexus-primary/5' : 'bg-slate-50/50 border-slate-100 opacity-50'
                            }`}
                          >
                            <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-xl border-2 border-dashed flex items-center justify-center ${isActive ? 'bg-white border-nexus-primary/30 text-nexus-primary' : 'bg-slate-50 border-slate-200 text-slate-300'}`}>
                                <FileText size={20} />
                              </div>
                              <div>
                                <div className={`font-bold text-lg ${isActive ? 'text-nexus-primary' : 'text-slate-400'}`}>
                      ...
                      {isActive && (
                        <div className="lg:col-span-4 space-y-4">
                           <div className="bg-gradient-to-br from-nexus-primary to-nexus-accent rounded-3xl p-5 text-white shadow-lg shadow-nexus-primary/20 relative overflow-hidden">
                              ...
                              <div className="space-y-2">
                                 <div className="text-[10px] font-bold uppercase tracking-wider opacity-60">אג׳נדה מומלצת:</div>
