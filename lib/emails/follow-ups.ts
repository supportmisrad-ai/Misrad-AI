...
function generateDay2CheckinEmailHTML(params: {
    ...
    const whatsappSection = whatsappUrl
        ? '<div style="text-align:center;margin:20px 0;"><a href="' + whatsappUrl + '" style="display:inline-block;background:#16a34a;color:white;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">וואטסאפ תמיכה</a></div>'
        : '';
    ...
    const bodyContent = `
        ...
        <div style="font-size:16px;line-height:1.6;color:#334155;margin-bottom:24px;">
            ראיתי שהתחלת עם <strong>"${params.organizationName}"</strong>.
            <br />
...
function generateDay7CheckinEmailHTML(params: {
    ...
    const whatsappSection = whatsappUrl
        ? '<div style="text-align:center;margin:20px 0;"><a href="' + whatsappUrl + '" style="display:inline-block;background:#16a34a;color:white;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">צריכים עזרה? דברו איתנו בוואטסאפ</a></div>'
        : '';
    ...
    const bodyContent = `
        ...
        <div style="font-size:16px;line-height:1.6;color:#334155;margin-bottom:24px;">
            עבר שבוע מאז שהתחלת עם <strong>"${params.organizationName}"</strong>.
            <br />
...
function generateDay45FeedbackEmailHTML(params: {
    ...
    const starsHtml = [1, 2, 3, 4, 5].map((n) => {
        ...
        const border = '#e2e8f0';
        const color = '#fbbf24';
        return '<td style="padding:0 6px;"><a href="' + params.ratingUrl + '?rating=' + n + '" style="display:inline-block;width:48px;height:48px;line-height:48px;border-radius:8px;background:' + bg + ';color:' + color + ';font-size:20px;font-weight:700;text-decoration:none;text-align:center;border:1px solid ' + border + ';">' + n + '</a></td>';
    }).join('');

    const googleSection = googleUrl
        ? '<div style="margin:28px 0;padding:20px;background:#f0fdf4;border-radius:8px;border:1px solid #bbf7d0;text-align:center;"><div style="font-size:14px;font-weight:700;color:#166534;margin-bottom:8px;">נהנים מ-MISRAD?</div><div style="font-size:13px;color:#15803d;margin-bottom:12px;">דירוג בגוגל עוזר לנו מאוד ומאפשר לעוד עסקים לגלות אותנו</div><a href="' + googleUrl + '" style="display:inline-block;background:#16a34a;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;font-size:13px;">דירוג בגוגל</a></div>'
        : '';
    ...
    const bodyContent = `
        ...
        <div style="font-size:16px;line-height:1.6;color:#334155;margin-bottom:24px;">
            עבר חודש וחצי מאז שהתחלת להשתמש ב-<strong>"${params.organizationName}"</strong>.
            <br />
        ...
        <div style="margin:28px 0;text-align:center;">
            <div style="font-size:14px;font-weight:700;color:#4f46e5;margin-bottom:12px;">דרג/י את החוויה שלך</div>
...
function generateDay30StatsEmailHTML(params: {
    ...
    const bodyContent = `
        ...
        <div style="font-size:16px;line-height:1.6;color:#334155;margin-bottom:24px;">
            חודש ראשון עם <strong>"${params.organizationName}"</strong> כבר מאחוריך!
            <br />
            הנה מה שהצלחת לעשות החודש:
        </div>

        <div style="margin:28px 0;padding:24px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;">
            <table role="presentation" style="width:100%;" cellpadding="0" cellspacing="0">
                <tr>
                    <td style="padding:12px 0;border-bottom:1px solid #e2e8f0;text-align:center;">
                        <div style="font-size:28px;font-weight:800;color:#4f46e5;">${clientCount}</div>
                        <div style="font-size:13px;color:#64748b;">לקוחות במערכת</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:12px 0;border-bottom:1px solid #e2e8f0;text-align:center;">
                        <div style="font-size:28px;font-weight:800;color:#4f46e5;">${taskCount}</div>
                        <div style="font-size:13px;color:#64748b;">משימות שבוצעו</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:12px 0;text-align:center;">
                        <div style="font-size:28px;font-weight:800;color:#10b981;">∞</div>
                        <div style="font-size:13px;color:#64748b;">זמן שנחסך</div>
                    </td>
                </tr>
            </table>
        </div>
