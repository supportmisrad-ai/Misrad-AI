...
import { markWorkListingInterested } from '@/app/actions/work-listings';
...
export default function OfferInterestedClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await markWorkListingInterested({
        id: String(id),
      ...
      if (!res.ok) {
        setError(res.message || 'שגיאה בשליחת הפנייה');
        return;
      }
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="text-sm font-bold text-emerald-800 mt-1">הפרטים נקלטו. יחזרו אליך בהקדם.</div>
          </div>
        </div>
      </div>
    );
