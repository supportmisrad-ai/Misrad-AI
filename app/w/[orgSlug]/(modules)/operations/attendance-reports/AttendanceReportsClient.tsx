...
export default function AttendanceReportsClient({
  ...
  orgSlug: string;
  initialReports: MonthlyReportData[];
  initialYear: number;
  initialMonth: number;
}) {
  ...
  const loadReports = useCallback(async () => {
    if (!orgSlug) return;
    if (isInitial && reports === initialReports) return;
    setIsLoading(true);
    try {
      const data = await listMonthlyReports(orgSlug, { year, month });
      setReports(data);
    } catch {
      setReports([]);
    } finally {
  ...
  }, [orgSlug, year, month, isInitial, reports, initialReports]);
  ...
  const handleGenerateMy = async () => {
    ...
    setMessage('');
    try {
      const report = await generateMonthlyReport(orgSlug, { year, month });
  ...
  const handleGenerateAll = async () => {
    ...
    setMessage('');
    try {
      const result = await generateAllMonthlyReports(orgSlug, { year, month });
  ...
  const openPdf = (reportId: string) => {
    window.open(`/api/attendance-report-pdf/${reportId}?orgSlug=${encodeURIComponent(orgSlug)}`, '_blank');
  };
