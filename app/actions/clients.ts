...
export async function getClients(
  ...
  try {
    ...
    if (orgId) {
      await requireWorkspaceAccessByOrgSlug(orgId);
    ...
    if (error) {
      ...
      if (isMissingClientsTable) {
        // Fallback for the new schema where the CRM clients are stored in misrad_clients
    ...
    const clients: Client[] = (data || []).map((client: any) => ({
      ...
      phone: client.phone,
      email: client.email,
      avatar: client.avatar || 'https://i.pravatar.cc/150?u=' + client.id,
