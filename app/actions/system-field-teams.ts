...
export async function getFieldTeamsAction(orgSlug: string): Promise<{ teams: FieldTeamDTO[]; error?: string }> {
  try {
    ...
    const teams = await prisma.misradFieldTeam.findMany({
    ...
    return {
      teams: teams.map((t) => ({
        ...
        agents: t.agents.map((a) => ({
          ...
          visits_today: a.visits.filter((v) => v.status === 'COMPLETED').length,
          visits_remaining: a.visits.filter((v) => v.status === 'UPCOMING' || v.status === 'CURRENT').length,
        })),
      })),
    };
  } catch (err) {
...
export async function addFieldAgentAction(orgSlug: string, data: {
  ...
  try {
    ...
    const agent = await prisma.misradFieldAgent.create({
      data: {
        organization_id: organizationId,
...
export async function deleteFieldTeamAction(orgSlug: string, teamId: string): Promise<{ error?: string }> {
  try {
    ...
    await prisma.misradFieldTeam.delete({ where: { id: teamId } });
    revalidatePath(`/w/${orgSlug}/system/field_map`, 'page');
    return {};
  } catch (err) {
...
export async function deleteFieldAgentAction(orgSlug: string, agentId: string): Promise<{ error?: string }> {
  try {
    ...
    await prisma.misradFieldAgent.delete({ where: { id: agentId } });
    revalidatePath(`/w/${orgSlug}/system/field_map`, 'page');
    return {};
  } catch (err) {
